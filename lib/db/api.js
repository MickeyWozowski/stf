...
const flashCancellableStatuses = [
  'queued'
, 'validating'
, 'preparing'
...
function normalizePlatformFamily(identity) {
  if (identity && identity.platformFamily) {
    return String(identity.platformFamily).toLowerCase()
  }

  var platform = String(identity && identity.platform || '').trim().toLowerCase()
  if (!platform) {
    return null
  }
...
function normalizeIosIdentity(identity) {
  var identityObj = identity || {}
  var ios = Object.assign({}, identityObj.ios || {})
  var simulator = null

  if (typeof ios.simulator === 'boolean') {
    simulator = ios.simulator
  }
  else if (typeof identityObj.iosSimulator === 'boolean') {
    simulator = identityObj.iosSimulator
  }

  var normalized = {
    udid: ios.udid || identityObj.iosUdid || null
  , name: ios.name || identityObj.iosName || null
  , productType: ios.productType || identityObj.iosProductType || null
  , productVersion: ios.productVersion || identityObj.iosProductVersion || null
  , simulator: simulator
  , transport: ios.transport || identityObj.iosTransport || null
  }

  var hasData = normalized.udid ||
    normalized.name ||
    normalized.productType ||
    normalized.productVersion ||
    normalized.transport ||
    typeof normalized.simulator === 'boolean'
...
dbapi.saveDeviceInitialState = function(serial, device) {
  ...
  , usage: null
  , logs_enabled: false
  , platformFamily: device.platformFamily || null
  , ios: device.ios || null
  }
...
dbapi.saveDeviceIdentity = function(serial, identity) {
  var ios = normalizeIosIdentity(identity)
  ...
  , platformFamily: normalizePlatformFamily(identity)
  , ios: ios
  }))
}
...
dbapi.createFlashJob = function(job) {
  const id = util.format('%s', uuid.v4()).replace(/-/g, '')
  const now = r.now()

  return db.run(r.table('flashJobs').insert({
      id: id
    , deviceSerial: job.deviceSerial
    , provider: job.provider || null
    , createdBy: job.createdBy
    , packageRef: job.packageRef
    , manifestVersion: job.manifestVersion || null
    , targetBuild: job.targetBuild || null
    , status: 'queued'
    , progress: 0
    , step: 'queued'
    , message: job.message || 'Flash job queued'
    , executionMode: job.executionMode || 'dry-run'
    , executionBackend: job.executionBackend || 'mac-dev-local'
    ...
    .then(function() {
      return dbapi.getFlashJob(id)
    })
}

dbapi.getFlashJob = function(id) {
  return db.run(r.table('flashJobs').get(id))
}

dbapi.listFlashJobs = function(options) {
  const opts = options || {}
  const limit = opts.limit || 20
  const status = opts.status
  const serial = opts.deviceSerial
  const createdBy = opts.createdBy
  const includeLogs = opts.includeLogs === true

  let query = r.table('flashJobs')

  if (status) {
    query = query.getAll(status, {index: 'status'})
  }
  if (serial) {
    query = status ?
      query.filter({deviceSerial: serial}) :
      r.table('flashJobs').getAll(serial, {index: 'deviceSerial'})
  }
  if (createdBy) {
    query = status || serial ?
      query.filter({createdBy: createdBy}) :
      r.table('flashJobs').getAll(createdBy, {index: 'createdBy'})
  }

  query = query.orderBy(r.desc('createdAt')).limit(limit)

  if (!includeLogs) {
    query = query.without('logLines')
  }
...
dbapi.listFlashJobsByDevice = function(serial, limit) {
  return db.run(r.table('flashJobs')
    .getAll(serial, {index: 'deviceSerial'})
...
dbapi.updateFlashJob = function(id, patch) {
  ...
  return db.run(r.table('flashJobs')
    ...
    .then(function(stats) {
      if (!stats.replaced && !stats.unchanged) {
        return null
      }
      if (stats.changes && stats.changes.length) {
        return stats.changes[0].new_val
      }
      return dbapi.getFlashJob(id)
    })
}

dbapi.setFlashJobStatus = function(id, status, patch) {
  const data = Object.assign({}, patch || {}, {
    status: status
  })
  return dbapi.updateFlashJob(id, data)
}

dbapi.appendFlashJobLog = function(id, line, limit) {
  ...
  function sanitizeArgs(args) {
    ...
    return args.slice(0, 64).map(function(value) {
      return clip(value, 512)
    })
  }

  function buildLogLine(value) {
    var entry = null
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      entry = {
          line: clip(typeof value.line === 'undefined' ? '' : value.line, 4096)
        , at: value.at ? new Date(value.at) : new Date()
        }

      if (value.level) {
        entry.level = clip(value.level, 32)
      }
      if (value.eventType) {
        entry.eventType = clip(value.eventType, 64)
      }
      if (value.stream) {
        entry.stream = clip(value.stream, 32)
      }
      if (value.commandId) {
        entry.commandId = clip(value.commandId, 128)
      }
      if (value.command) {
        entry.command = clip(value.command, 512)
      }
      if (Array.isArray(value.args)) {
        entry.args = sanitizeArgs(value.args)
      }
      if (typeof value.pid === 'number' && isFinite(value.pid)) {
        entry.pid = Math.floor(value.pid)
      }
      if (value.phase) {
        entry.phase = clip(value.phase, 64)
      }
      if (typeof value.code === 'number' && isFinite(value.code)) {
        entry.code = Math.floor(value.code)
      }
      if (value.signal) {
        entry.signal = clip(value.signal, 64)
      }
      if (typeof value.durationMs === 'number' && isFinite(value.durationMs)) {
        entry.durationMs = Math.floor(value.durationMs)
      }
      if (value.step) {
        entry.step = clip(value.step, 64)
      }
      if (value.actionType) {
        entry.actionType = clip(value.actionType, 128)
      }
      if (value.partition) {
        entry.partition = clip(value.partition, 128)
      }
      if (value.artifactId) {
        entry.artifactId = clip(value.artifactId, 128)
      }
      return entry
    }

    return {
        line: clip(value, 4096) || ''
      , at: new Date()
      }
  }

  const maxLines = limit || 400
  const logLine = buildLogLine(line)

  return db.run(r.table('flashJobs')
    .get(id)
    .update({
      logLines: r.row('logLines').default([]).append(logLine).slice(-maxLines)
    , updatedAt: r.now()
    }))
}

dbapi.claimNextQueuedFlashJob = function(provider, workerName) {
  return db.run(r.table('flashJobs')
    .getAll('queued', {index: 'status'})
    .orderBy('createdAt')
    .limit(1)
    .update(function(row) {
      return r.branch(
        row('status').eq('queued')
      , {
          status: 'validating'
        , step: 'claiming'
        , message: 'Flash job claimed by worker'
        , provider: provider || row('provider').default(null)
    ...
    .then(function(stats) {
      if (!stats.replaced || !stats.changes || !stats.changes.length) {
        return null
      }
      return stats.changes[0].new_val
    })
...
dbapi.cancelFlashJob = function(id, actor, reason) {
  ...
  return db.run(r.table('flashJobs')
    ...
    .update(function(row) {
      return r.branch(
        r.expr(flashCancellableStatuses).contains(row('status'))
      , {
          status: 'canceled'
        , step: 'canceled'
        ...
        , progress: row('progress').default(0)
        , canceledAt: r.now()
        , updatedAt: r.now()
        }
      , {}
...
dbapi.getSamsungFlashServiceStatus = function(provider) {
  ...
  function withProviderFilter(query) {
    if (!providerName) {
      return query
    }
    return query.filter(function(job) {
      return job('provider').default(null).eq(providerName)
    })
  }

  function countByStatus(status) {
    return db.run(withProviderFilter(
      r.table('flashJobs').getAll(status, {index: 'status'})
    ).count())
  }

  function getLatestJob() {
    return db.run(withProviderFilter(r.table('flashJobs'))
  ...
  return Promise.props({
    ...
    , validating: countByStatus('validating')
    , preparing: countByStatus('preparing')
    , flashing: countByStatus('flashing')
    , verifying: countByStatus('verifying')
    ...
    .then(function(stats) {
      var active = stats.validating + stats.preparing + stats.flashing + stats.verifying
      var queued = stats.queued
      var operatingState = 'idle'
      ...
      else if (stats.latest && stats.latest.status === 'failed') {
      ...
      return {
        ...
        , latestJob: stats.latest ? {
            id: stats.latest.id
          , status: stats.latest.status
          , step: stats.latest.step
          , message: stats.latest.message
          , updatedAt: stats.latest.updatedAt
          , deviceSerial: stats.latest.deviceSerial
          , executionBackend: stats.latest.executionBackend || null
          , executionMode: stats.latest.executionMode || null
          } : null
        }
    })
}
