    ...
    new Promise(function(resolve) {
      ...
      socket.on('disconnect', resolve)
        ...
        .on('input.touchDown', function(channel, data) {
          log.debug(
            'input.touchDown channel=%s seq=%d contact=%d x=%s y=%s pressure=%s'
          , channel
          , data.seq
          , data.contact
          , data.x
        ...
        .on('device.flash-samsung.enqueue', function(channel, responseChannel, data) {
          ...
          if (!data || !data.serial) {
            cancel('INVALID_REQUEST', 'Missing device serial')
            return
          }

          var executionMode = data.executionMode === 'execute' ? 'execute' : 'dry-run'
          var executionBackend = data.executionBackend || 'mac-dev-local'
          var metadata = Object.assign({}, data.metadata || {})
          var providerName = null

          if (typeof data.manifestPath === 'string' && data.manifestPath.trim().length) {
            metadata.manifestPath = data.manifestPath.trim()
          }

          if (typeof data.packageBasePath === 'string' && data.packageBasePath.trim().length) {
            metadata.packageBasePath = data.packageBasePath.trim()
          }

          if (typeof data.destructiveConfirmation === 'string' && data.destructiveConfirmation.trim().length) {
            metadata.destructiveConfirmation = data.destructiveConfirmation.trim()
          }

          if (typeof data.simulate !== 'undefined') {
            metadata.simulate = data.simulate === true
          }

          if (data.deviceInfo && typeof data.deviceInfo === 'object') {
            metadata.deviceInfo = Object.assign({}, metadata.deviceInfo || {}, data.deviceInfo)
          }

          dbapi.loadDeviceBySerial(data.serial)
            .then(function(device) {
              if (!device) {
                cancel('DEVICE_NOT_FOUND', 'Device not found in STF database')
                return null
              }

              if (device.channel && channel && device.channel !== channel) {
                cancel('CHANNEL_MISMATCH', 'Device channel mismatch for requested serial')
                return null
              }

              providerName = data.provider || (device.provider && device.provider.name) || null

              return dbapi.createFlashJob({
                  deviceSerial: data.serial
                , provider: providerName
                , createdBy: user.email
                , packageRef: data.packageRef || 'firmware://sm-t830/xar/u5'
                , manifestVersion: data.manifestVersion || null
                , targetBuild: data.targetBuild || null
                , message: data.message || 'Flash job queued from engineering menu'
            ...
            .catch(function(err) {
              log.error('Failed to enqueue Samsung flash job from websocket: %s', err.stack || err.message || err)
              cancel('FLASH_JOB_CREATE_FAILED', err.message || String(err))
            })
        })
        .on('service.flash-samsung.status', function(channel, responseChannel, data) {
          function done(payload) {
            socket.emit('tx.done', responseChannel, {
                seq: 0
              , source: channel
          ...
          var provider = data && typeof data.provider === 'string' ? data.provider : null
          ...
          dbapi.getSamsungFlashServiceStatus(provider)
            ...
            .catch(function(err) {
              log.error(
                'Failed to retrieve Samsung flash service status: %s'
              , err.stack || err.message || err
              )
              cancel('FLASH_SERVICE_STATUS_FAILED', err.message || String(err))
            })
        })
        ...
        .on('device.launchApp', function(channel, responseChannel, data) {
          joinChannel(responseChannel)
          push.send([
            channel
          , wireutil.transaction(
              responseChannel
            , new wire.LaunchActivityMessage(
                'ios.launch'
              , data.bundleId
              , []
              , 0
